...
function Test-Jobs([string]$Binary) {
    $helpText = & $Binary jobs run --help 2>$null
    if ($LASTEXITCODE -ne 0 -or (($helpText -join "`n") -notmatch "(?m)^Usage: archdev jobs run ")) { return $false }
...
if (-not (Test-Jobs $archdev)) {
    [Console]::Error.WriteLine("Updating ArchDev because this version lacks Jobs automation commands.")
...
if (-not (Test-Jobs $archdev)) { throw "Installed ArchDev does not provide Jobs automation commands" }
