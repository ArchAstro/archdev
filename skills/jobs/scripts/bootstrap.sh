...
if ! "$executable" jobs setup --help >/dev/null 2>&1; then
  printf 'Updating ArchDev because this version lacks the Jobs domain.\n' >&2
  install_archdev
  executable="$(absolute_path "$install_dir/archdev")"
fi
...
"$executable" jobs setup --help >/dev/null 2>&1 || {
  printf 'Installed ArchDev does not provide the Jobs domain.\n' >&2
  exit 1
}
printf '%s\n' "$executable"
